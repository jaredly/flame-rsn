...
type attractor =
  ...
  | P1 (float => fn)
  | P2 (float => float => fn)
  | P3 (float => float => float => fn)
  | P4 (float => float => float => float => fn)
  | P6 (float => float => float => float => float => float => fn)
;
...
let slide default fn node => {
  [%map let fn = fn node
  and a = slider node (-8.) 8. 0.01 default];
  fn a
};
let slide_ range default fn node => {
  [%map let fn = fn node
  and a = slider node (-. range) range 0.01 default];
  fn a
};
...
fn |> wrap |> slide_ 1. a |> slide_ 1. b |> slide_ 1. c |> slide_ 1. d |> slide_ 1. e |> slide_ 1. f
  ;

run (slide6 0.5 0. 0.5 0. 1. 0. affine); /* (x + 1)/2, y */
